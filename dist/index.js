    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            try {
                ...
                throw new Error('Cannot parse JSON secrets. Be sure to set secrets: ${{ toJSON(secrets) }}');
            }
            core.debug(`Current keys of secrets: ${Object.keys(secrets).join(', ')}`);
            const includeList = includeRegexpStr.split(',').map(key => key.trim());
            const excludeList = [
                'GITHUB_TOKEN',
                ...excludeRegexpStr.split(',').map(key => key.trim()),
            ];
            const replaceObject = replaceRegexpStr.split(',').map(key => key
                .trim()
                .split('=')
                .map(val => val.trim()));
            ...
            core.info(`Generated file to ${envPath}. (keys: ${Object.keys(secrets).join(', ')})`);
        }
        catch (error) {
